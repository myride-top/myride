...
const FullscreenPhotoViewer = dynamic(
  () =>
    import('@/components/photos/fullscreen-photo-viewer').then(
      (mod) => mod.FullscreenPhotoViewer
    ),
  { ssr: false }
)

const CarComments = dynamic(
  () => import('@/components/cars/car-comments').then((mod) => mod.CarComments),
  { ssr: false }
)

const CarTimeline = dynamic(
  () => import('@/components/cars/car-timeline').then((mod) => mod.CarTimeline),
  { ssr: false }
)
...
export default function CarDetailPage() {
  ...
  const handleLike = async () => {
    ...
    if (user.id === car.user_id) {
      toast.error(t('carDetail.toast.likeOwn', 'You cannot like your own car'))
      return
    }
    ...
    try {
      if (isLiked) {
        ...
        setIsLiked(false)
        toast.success(t('carDetail.toast.unliked', 'Car unliked'))
      } else {
        await likeCarClient(car.id, user.id)
        setIsLiked(true)
        toast.success(t('carDetail.toast.liked', 'Car liked!'))
      }
      ...
      toast.error(
        t('carDetail.toast.likeUpdateFailed', 'Failed to update like status')
      )
    } finally {
                  ...
                  {profile?.is_premium ? (
                    <Link
                      ...
                      title={t(
                        ...
                        `View ${profile.full_name || profile.username}'s garage`
                      )}
                    >
                      <UserAvatar
                      ...
                      <span className='flex items-center gap-1 hover:text-foreground transition-colors text-sm md:text-base'>
                        ...
                        <span className='truncate'>
                          @{profile.username || params.username || t('common.unknown', 'Unknown')}
                        </span>
                        {profile.nationality && (
                          <NationalityFlag
                ...
                <div className='flex items-center gap-1.5 lg:hidden'>
                  ...
                  {user && car && user.id !== car.user_id ? (
                    <button
                      ...
                      title={
                        isLiked
                          ? t('carDetail.unlikeCar', 'Unlike car')
                          : t('carDetail.likeCar', 'Like car')
                      }
                  ...
                  <button
                    ...
                    title={t(
                      'carDetail.shareQrTitle',
                      `Share ${car.name} via QR Code`
                    )}
              ...
              >
                ...
                <span className='text-sm font-medium text-foreground'>
                  {likeCount > 0
                    ? `${likeCount} ${
                        likeCount !== 1
                          ? t('carDetail.likesPlural', 'likes')
                          : t('carDetail.likeSingular', 'like')
                      }`
                    : t('carDetail.like', 'Like')}
                </span>
              </button>
            ) : (
              likeCount > 0 && (
            ...
            <div className='lg:col-span-2 order-first'>
              ...
              {car.photos && car.photos.length > 0 && (
                <div className='mb-4 sm:mb-6'>
                  <div className='flex flex-wrap gap-2'>
                    ...
                    >
                      {t('carDetail.photos.all', 'All')} ({car.photos.length})
                    </button>
                    {PHOTO_CATEGORIES.map(category => {
                      ...
                      return (
                        ...
                        >
                          {t(`photo.category.${category}`, category)} ({count})
                        </button>
                      )
                    })}
                  </div>
              ...
              sortedPhotos[selectedPhoto] ? (
                  ...
                  <div className='relative'>
                    <div className='relative w-full h-64 sm:h-80 md:h-96 rounded-lg shadow-lg overflow-hidden'>
                      <Image
                        ...
                        alt={`${car.name} - ${
                          t(
                            `photo.category.${getPhotoInfo(sortedPhotos[selectedPhoto]).category}`,
                        ...
                        >
                          <div className='relative w-full h-full'>
                            <Image
                              ...
                              alt={`${car.name} ${t(
                                `photo.category.${getPhotoInfo(photo).category}`,
                ...
                <EmptyState
                  ...
                  title={
                    car.photos && car.photos.length > 0
                      ? t('carDetail.photos.corrupted', 'Photos Corrupted')
                      : t('carDetail.photos.none', 'No photos yet')
                  }
                  description={
                    car.photos && car.photos.length > 0
                      ? t(
                          'carDetail.photos.corruptedDescription',
                          'The existing photos appear to be corrupted. Please re-upload them in the edit page.'
                        )
                      : selectedCategory === 'all'
                      ? t(
                          'carDetail.photos.noneDescription',
                          'No photos have been uploaded for this car.'
                        )
                      : t(
                          'carDetail.photos.noneCategory',
