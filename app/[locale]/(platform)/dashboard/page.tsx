...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        toast.error(
          t('dashboard.error.loadFailed', 'Failed to load dashboard data')
        )
      } finally {
        setLoading(false)
      }
  ...
  const handleShareProfile = async () => {
    ...
    if (!profile?.username) {
      toast.error(t('dashboard.error.usernameMissing', 'Username not found'))
      return
    }
    ...
    if (!qrCodeDataUrl) {
      ...
      try {
        ...
        toast.error(
          t('dashboard.error.qrFailed', 'Failed to generate QR code')
        )
      } finally {
  ...
  const dashboardStats: DashboardStat[] = [
    {
      icon: Heart,
      label: t('dashboard.stats.totalLikes', 'Total Likes'),
    ...
    {
      icon: Share2,
      label: t('dashboard.stats.totalShares', 'Total Shares'),
    ...
    {
      icon: MessageCircle,
      label: t('dashboard.stats.totalComments', 'Total Comments'),
      ...
      <PageLayout showCreateButton={false}>
        <PageHeader
          ...
          description={t(
            ...
            'Manage your cars and track your performance'
          )}
        />
          ...
          <div className='mb-6 md:mb-8'>
            <div className='flex items-center justify-between mb-3 md:mb-4'>
              <h2 className='text-lg md:text-xl font-semibold text-foreground'>
                {t('dashboard.performanceOverview', 'Performance Overview')}
              </h2>
              {carSlots.isPremium && (
                <Link
                  href='/analytics'
        ...
        {!carSlots.isPremium && cars.length >= carSlots.maxAllowedCars && (
          <PremiumUpgradeBanner
            ...
            className='mb-6'
            variant={
              carSlots.maxAllowedCars - cars.length === 0
                ? 'compact'
                : 'default'
            }
          />
        ...
        <div>
          <div className='flex items-center justify-between gap-2 mb-6'>
              ...
              {t('dashboard.yourCars', 'Your Cars')}
              </h2>
                ...
                <>
                  {carSlots.isPremium ? (
                    <button
                      ...
                      title={t('dashboard.shareGarage', 'Share your garage')}
                    >
                      {isGeneratingQR ? (
                        <>
                        ...
                        <>
                          ...
                          <span className='truncate'>
                            {t('dashboard.shareGarage', 'Share Garage')}
                          </span>
                        </>
                      )}
                    </button>
                    ...
                    <PremiumButton
                      featureName={t('dashboard.shareGarage', 'Share Garage')}
                      featureDescription={t(
                        'dashboard.shareGarageDescription',
                        'Share your entire garage with a custom URL and QR code. Make your profile public and shareable.'
                      )}
                    >
                      {t('dashboard.shareGarage', 'Share Garage')}
                    </PremiumButton>
          ...
          {cars.length === 0 ? (
            <EmptyState
              icon={Info}
              title={t('dashboard.empty.title', 'No cars yet')}
              description={t(
              ...
              action={
                ...
                >
                  ...
                  {t('dashboard.empty.action', 'Add Your First Car')}
                </Link>
              }
            />
          ) : (
