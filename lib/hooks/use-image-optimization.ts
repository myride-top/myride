...
export function useImageOptimization({
  ...
  const optimizeImage = useCallback(async (file: File): Promise<OptimizedImage> => {
    ...
    try {
      ...
      setProgress(prev => ({
        ...
        [fileName]: {
          status: 'optimized',
          details: `${formatFileSize(originalSize)} → ${formatFileSize(optimizedFile.size)} (${compressionRatio.toFixed(1)}% smaller)`,
          originalSize,
      ...
      if (showToasts) {
        toast.success(
          `${fileName} optimized: ${formatFileSize(originalSize)} → ${formatFileSize(optimizedFile.size)}`
        )
      }
  ...
  const optimizeImagesWithProgress = useCallback(async (
    files: File[],
  ...
  const getOptimizationStats = useCallback(() => {
    ...
    const optimizedFiles = entries.filter(p => p.status === 'optimized' && p.wasOptimized).length
    const failedFiles = entries.filter(p => p.status === 'failed').length
    const totalOriginalSize = entries.reduce((sum, p) => sum + p.originalSize, 0)
    const totalOptimizedSize = entries.reduce((sum, p) => sum + (p.optimizedSize || p.originalSize), 0)
...
export function useCarPhotoOptimization(options?: UseImageOptimizationOptions) {
  ...
  const optimizeCarPhoto = useCallback(async (file: File) => {
  ...
  const optimizeCarPhotos = useCallback(async (files: File[]) => {
